...
Lukas Novotny, Bert Hecht - Principles of Nano-Optics - Cambridge University Press (2012)
ISBN 978-1-107-00546-4
Section 2.12, Section 8.3, Appendix D
Here, we introduce the presence of a hemipherical mirror in the far-field
*)
ClearAll["Global`*"]
(*-------------------------------------*)
(*COORDINATES*)
...
(* ----------------------------------------------------
The full expression is (ISBN 978-1-107-00546-4, pag.29 Eq. 2.94):
Gt = i*\[Omega]*(I * G0 + (1/k^2)*Grad(Div(I*G0))),
where all the coefficients are already in the Green's function.
i*\[Omega]*(I * G0) is dA/dt and
(1/k^2)*Grad(Div(I*G0)) originates from -Grad[\[Phi]]=-Grad[Div[A]] in Lorentz's gauge
...
The current that is generating the field is
j(r) = q*dx/dt = \[Minus]i\[Omega]p\[Delta][r].
where p is the dipole moment.
The light intensity is given by
I = (\[Epsilon]0*n*c/2) * |E|^2
where n is the index of refraction. The factor 1/2 comes from the time average.
*)
simconditions = {\[Mu]>0, \[Mu]0>0, \[Omega]>0, r0>0, \[Theta]0>0, \[Phi]0>0, k>0, p>0};
jvet =-I*\[Omega]*p{1,0,0}
dId\[CapitalOmega][jvet_,rr0_,\[Theta]\[Theta]0_,\[Phi]\[Phi]0_]:=Simplify[(Gtr\[Theta]\[Phi][k,0,0,0,0,0,r0,\[Theta]0,\[Phi]0].jvet).Conjugate[Gtr\[Theta]\[Phi][k,0,0,0,0,0,r0,\[Theta]0,\[Phi]0].jvet], Assumptions->simconditions]/.{r0->rr0,\[Theta]0->\[Theta]\[Theta]0,\[Phi]0->\[Phi]\[Phi]0};
Intensity[jvet_]:=(\[Epsilon]0*c/2)*Integrate[Integrate[dId\[CapitalOmega][jvet,r0,\[Theta]0,\[Phi]0]*Sin[\[Theta]0],{\[Theta]0,0,\[Pi]}],{\[Phi]0,0,2*\[Pi]}]/.{\[Mu]->1,\[Mu]0->1/(\[Epsilon]0*c^2)}
Print["Radiated power \[Pi] x-pol = ", Intensity[-I*\[Omega]*p*{1,0,0}]]
Print["Radiated power \[Pi] y-pol = ", Intensity[-I*\[Omega]*p*{0,1,0}]]
Print["Radiated power \[Pi] z-pol = ", Intensity[-I*\[Omega]*p*{0,0,1}]]
Print["Radiated power \[Sigma] x-pol = ", Intensity[-(I*\[Omega]*p/Sqrt[2])*{0,1,I}]]
Print["Radiated power \[Sigma] y-pol = ", Intensity[-(I*\[Omega]*p/Sqrt[2])*{1,0,I}]]
Print["Radiated power \[Sigma] z-pol = ", Intensity[-(I*\[Omega]*p/Sqrt[2])*{1,I,0}]]
(*plotting*)
\[Omega]=1;
\[Mu]0=1;
\[Mu]=1;
\[Epsilon]0=1;
p=1;
dId\[CapitalOmega][-I*\[Omega]*p*{1,0,0},1,\[Theta],\[Phi]]
\[Pi]xpolplot = SphericalPlot3D[dId\[CapitalOmega][-I*\[Omega]*p*{1,0,0},1,\[Theta],\[Phi]],{\[Theta],0,\[Pi]},{\[Phi],0,2*\[Pi]}];
\[Pi]ypolplot = SphericalPlot3D[dId\[CapitalOmega][-I*\[Omega]*p*{0,1,0},1,\[Theta],\[Phi]],{\[Theta],0,\[Pi]},{\[Phi],0,2*\[Pi]}];
\[Pi]zpolplot = SphericalPlot3D[dId\[CapitalOmega][-I*\[Omega]*p*{0,0,1},1,\[Theta],\[Phi]],{\[Theta],0,\[Pi]},{\[Phi],0,2*\[Pi]}];
\[Sigma]xpolplot = SphericalPlot3D[dId\[CapitalOmega][-(I*\[Omega]*p/Sqrt[2])*{0,1,I},1,\[Theta],\[Phi]],{\[Theta],0,\[Pi]},{\[Phi],0,2*\[Pi]}];
